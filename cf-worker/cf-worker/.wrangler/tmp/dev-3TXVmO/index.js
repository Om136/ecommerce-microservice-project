...
// .wrangler/tmp/bundle-PDA9I8/checked-fetch.js
...
var jwtAuth = /* @__PURE__ */ __name((options = {}) => {
...
// src/middleware/cache-strategies.js
var isCacheable = /* @__PURE__ */ __name((request, response) => {
...
var createCacheStrategy = /* @__PURE__ */ __name((options = {}) => {
  const {
    ...
    bypassCache = false,
    // Flag to bypass cache for debugging
    varyByHeaders = []
  ...
  return async (c, next) => {
    ...
    if (varyByHeaders.length > 0) {
      const varyParams = varyByHeaders.map((header) => `${header}=${c.req.header(header) || "none"}`).join("&");
...
var productListCache = createCacheStrategy({
...
var productDetailCache = createCacheStrategy({
...
var staticAssetCache = createCacheStrategy({
...
var categoryCache = createCacheStrategy({
...
app.use(
  ...
  jwtAuth({
    excludePaths: [
      "/api/auth/login",
      "/api/user/login",
      // Added correct login endpoint
      "/api/auth/register",
      "/api/user/register",
      // Added user register endpoint
      "/api/products",
...
var logAnalytics = /* @__PURE__ */ __name(async (c, next) => {
...
app.all("*", async (c) => {
  ...
  try {
    ...
    const request = new Request(targetUrl, {
      ...
      body: ["GET", "HEAD"].includes(c.req.method) ? void 0 : await c.req.arrayBuffer(),
      redirect: "follow"
    });
...
// .wrangler/tmp/bundle-PDA9I8/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-PDA9I8/middleware-loader.entry.ts
